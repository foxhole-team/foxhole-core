...
| 3 | [Updates and data delivery](03-updates-and-data-delivery.md) | the signed feeds (four published, a fifth staged), manifest + detached signature, the pinned key, rollback protection, how a document reaches the running VPN, and the APK update channel |
...
```mermaid
flowchart LR
    A["foxhole_guard<br/>config/foxcore-revision.txt<br/>d7127c30…257b"] -->|"CI + F-Droid recipe<br/>check out that commit"| B["foxhole-core"]
...
| Client → core, revision | `config/foxcore-revision.txt` — enforced by `.github/actions/setup-foxcore/action.yml:22` and `metadata/com.foxhole.guard.yml:70,96`. **Not** enforced by a local Gradle build, which uses whatever is checked out |
...
| Document | Section |
|---|---|
| 2 — Cryptography | 2.7 — stale REALITY fingerprint capability comment, README profile count |
| 3 — Updates | 3.7 — duplicated pin literal, inline vs shared verifiers, asymmetric caps |
| 4 — Boundary | 4.7 — six unwired ABI exports, two divergent link parsers, unhandled reload code `-9` |
