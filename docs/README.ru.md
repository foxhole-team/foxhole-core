...
> читаемой части JA4. **Обычный путь TLS браузерным не является:** он использует
> собственный hello от rustls — измерено 10 шифронаборов и 11 расширений против
> 15 и 16 у Chromium, — то есть отличим от браузера. Ничего из измеренного не
> говорит, что цензор сегодня опирается на это. Конфигурацией это не лечится:
...
| **Web Apps / leases** | изолированная identity Web App, маршрутизируемые leases, origin-bound уведомления |
| **Обмен файлами (в разработке)** | нативное XChaCha20-Poly1305-хранилище, capability-доступ и примитивы onion-публикации; не является выпущенной функцией FoxHole Guard |
...
Рядом существуют ещё два ограниченных потока со своими счётчиками `dropped`:
события open/close карты трафика и события компонентов.
...
Хранилище файлов и примитивы onion-публикации скомпилированы в поставляемое
ядро, но функция остаётся **в разработке**: она ещё не прошла внешнюю сквозную
релизную проверку и недоступна в публичном приложении FoxHole Guard.
`share.compiled` означает наличие кода, а не готовность продукта.
...
- замороженные ABI-v1 C/JNI exports и совместимость capabilities/config;
  export-набор release ELF проверяется для обеих поставляемых ABI.
