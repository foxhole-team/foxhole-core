...
mod tests {
    ...
    fn test_extract_certificate_der_valid() {
        ...
        let mut message = vec![
            ...
            ((body_len >> 16) & 0xff) as u8, // Handshake length (3 bytes, big-endian)
            ((body_len >> 8) & 0xff) as u8,
            (body_len & 0xff) as u8,
            0x00,                            // certificate_request_context length = 0
            ((list_len >> 16) & 0xff) as u8, // certificate_list length (3 bytes)
            ((list_len >> 8) & 0xff) as u8,
            (list_len & 0xff) as u8,
            ((cert_len >> 16) & 0xff) as u8, // First certificate: cert_data length (3 bytes)
