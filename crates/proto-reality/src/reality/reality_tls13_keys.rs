...
fn hkdf_expand_label_with_algorithm(
    ...
    let encoded_length = u16::try_from(length)
        .map_err(|_| Error::new(ErrorKind::InvalidInput, "HKDF output length exceeds u16"))?;
    let full_label_len = b"tls13 "
        .len()
        .checked_add(label.len())
        .filter(|length| *length <= u8::MAX as usize)
        .ok_or_else(|| Error::new(ErrorKind::InvalidInput, "HKDF label is too long"))?;
    let context_len = u8::try_from(context.len())
        .map_err(|_| Error::new(ErrorKind::InvalidInput, "HKDF context is too long"))?;

    // Length (2 bytes, big-endian)
    hkdf_label.extend_from_slice(&encoded_length.to_be_bytes());

    // Label length and content. TLS labels are opaque bytes, not UTF-8.
...
/// * `shared_secret` - the negotiated group's `(EC)DHE` output: 32 bytes for
///   x25519, 64 for X25519MLKEM768 (`ml_kem_ss ‖ x25519_ss`). RFC 8446 feeds
///   this into HKDF-Extract as opaque IKM, so the only thing that matters here
///   is that it is the length the group defines.
...
pub fn derive_handshake_keys(
    ...
    // Validate input lengths. 32 is x25519; 64 is X25519MLKEM768's
    // `ml_kem_ss ‖ x25519_ss`. Anything else is a group this build does not
    // execute, and an empty or short secret would silently produce a key
    // schedule with no entropy in it.
