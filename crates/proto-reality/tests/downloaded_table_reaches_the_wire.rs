//! The tap itself: a verified table document supplies the bytes on the socket.
//!
//! `runtime_tables`' unit tests prove a document parses into the right table.
//! They cannot prove the last step, and the last step is exactly where this
//! whole feature was broken before: a value that is fetched, verified, stored —
//! and then never read by the thing that writes bytes. So this test reads the
//! socket.
//!
...
/// One test function, not two, because the assertions are ordered: what the
/// built-in tables send has to be established *before* a document is installed,
/// and the registry is process-global.
#[tokio::test]
async fn an_installed_table_changes_what_the_socket_carries() {
    // ---- before: the built-in tables are writing -----------------------
    ...
    // ---- install a document that says something different --------------
    ...
    // ---- after: the downloaded bytes are on the wire -------------------
    ...
    for profile in RealityHelloProfile::ALL {
        ...
        match support::extension_body(&hello, support::RENEGOTIATION_INFO) {
            ...
            // A profile the document carries no table for keeps its built-in
            // one. That is the promise, not a gap.
            Some(body) => assert_eq!(
                body,
                vec![0x00],
    ...
    // The hello is still a parrot, not merely different bytes: the same
    // extensions are present. A table supplies values; it cannot reshape the
    // hello into something no browser sends. Compared as a sorted set with
    // GREASE removed, because the Chromium profiles permute their extensions
    // and redraw their GREASE points on every connection by design.
